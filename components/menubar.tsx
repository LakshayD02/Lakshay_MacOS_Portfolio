...
export default function Menubar({
  ...
  useEffect(() => {
    const getBatteryApi = async () => {
      try {
        const batteryApi = (navigator as any).getBattery?.()
          || Promise.resolve((navigator as any).battery || (navigator as any).mozBattery || (navigator as any).webkitBattery)

        const battery = await batteryApi
        if (!battery) {
          throw new Error("Battery API unsupported")
        }
  ...
  return (
    ...
    >
      <div className="flex-1 flex items-center">
        ...
        {activeMenu === "apple" && (
          <div className={`absolute top-6 left-2 ${dropdownBgClass} rounded-lg shadow-xl ${textClass} py-1 w-56`}>
            <button className={`w-full text-left px-4 py-1 ${hoverClass}`} onClick={onOpenAbout}>
              About This Mac
            </button>
            <div className="border-t border-gray-700 my-1"></div>
            <button className={`w-full text-left px-4 py-1 ${hoverClass}`} onClick={onOpenSettings}>
              System Settings...
            </button>
            <button className={`w-full text-left px-4 py-1 ${hoverClass}`} onClick={onOpenAppStore}>
              App Store...
            </button>
            <div className="border-t border-gray-700 my-1"></div>
      ...
      <div className="flex items-center space-x-3">
        <span className="mr-1">{batterySupported ? (batteryLevel === null ? "--" : `${batteryLevel}%`) : "N/A"}</span>
        <div className="relative">
          <div className="w-6 h-3 border border-current rounded-sm relative">
            <div className="absolute top-0 left-0 bottom-0 bg-current" style={{ width: `${batteryLevel ?? 0}%` }}></div>

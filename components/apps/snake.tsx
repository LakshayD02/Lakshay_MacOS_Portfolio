...
export default function Snake({ isDarkMode = true }: SnakeProps) {
  ...
  // Handle keyboard input and prevent scrolling
  useEffect(() => {
  ...
  // Prevent touch scrolling on mobile controls
  useEffect(() => {
    ...
    buttons.forEach(button => {
      button.addEventListener('touchmove', preventTouchScroll, { passive: false })
      button.addEventListener('touchstart', preventTouchScroll, { passive: false })
    })

    return () => {
      buttons.forEach(button => {
        button.removeEventListener('touchmove', preventTouchScroll)
        button.removeEventListener('touchstart', preventTouchScroll)
      })
    }
  }, [])

  // Prevent scrolling on the entire page when game is active (not paused and not game over)
  useEffect(() => {
    if (!isPaused && !gameOver) {
      ...
      return () => {
        // Re-enable scrolling when component unmounts or game is paused/over
        document.body.style.overflow = ''
        document.body.style.position = ''
        document.body.style.width = ''
      }
    } else {
      // Re-enable scrolling when game is paused or game over
      document.body.style.overflow = ''
      document.body.style.position = ''
      document.body.style.width = ''
    }
